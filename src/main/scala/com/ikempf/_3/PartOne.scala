...
case class Claim(id: String, left: Int, top: Int, width: Int, height: Int) {
...
object PartOne extends App {
  ...
  def possiblePositions(claims: List[Claim]) = {
    val startLeft = claims.map(_.left).min
    val startTop  = claims.map(_.top).min
    val stopLeft  = claims.map(_.right).max
    val stopTop   = claims.map(_.bottom).max

    for {
      i <- Range.inclusive(startLeft, stopLeft)
      j <- Range.inclusive(startTop, stopTop)
    } yield (i, j)
  }
  ...
  def input =
    """#1 @ 808,550: 12x22
      ...
      |#1279 @ 262,28: 11x29
      |#1280 @ 824,23: 26x20
      |#1281 @ 755,745: 10x19""".stripMargin
